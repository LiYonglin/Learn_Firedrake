...
p12=p2-p1;
p23=p3-p2;
cw=(p12(1,:).*p23(2,:)-p12(2,:).*p23(1,:))<0; % clockwise
t([1,2],cw)=t([2,1],cw);

% Print to msh file
fname=[meshname '_nodes.txt'];
writematrix(p.',fname,'Delimiter',' ');
fname=[meshname '_elements.txt'];
writematrix(t(1:3,:).',fname,'Delimiter',' ');
triangulation_to_gmsh(meshname);
delete([meshname '_nodes.txt']);
delete([meshname '_elements.txt']);
```
```
function triangulation_to_gmsh ( prefix )

%*****************************************************************************80
%
%% MAIN is the main program for TRIANGULATION_TO_GMSH.
%
%  Discussion:
%
%    TRIANGULATION_TO_GMSH converts a triangulation, using 3 or 6 node 
%    triangular elements, to a Gmsh file.
%
%  Usage:
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'TRIANGULATION_TO_GMSH\n' );
  fprintf ( 1, '  MATLAB version\n' );
  fprintf ( 1, ...
    '  Convert a linear or quadratic triangulation to Gmsh format.\n' );
  fprintf ( 1, '\n' );
...
%
  ...
  elseif ( element_order == 6 )
    ...
    fprintf ( 1, '\n' );
    fprintf ( 1, 'TRIANGULATION_TO_GMSH - Fatal error!\n' );
    fprintf ( 1, '  Only 3-node and 6-node triangulations can be handled!\n' );
    error ( 'TRIANGULATION_TO_GMSH - Fatal error!' );
  end
...
%
  gmsh_write ( gmsh_filename, dim_num, node_num, node_xy, ...
...
function gmsh_write ( gmsh_filename, dim_num, node_num, node_xy, ...
  element_order, element_num, element_node )

%*****************************************************************************80
%
%% GMSH_WRITE writes the triangulation data as a Gmsh mesh file.
%
%  Licensing:
%
%    This code is distributed under the GNU LGPL license.
...
%  Parameters:
%
%    Input, string GMSH_FILENAME, the name of the Gmsh file to create.
%
...
%
  ...
  if ( gmsh < 0 ) 
    fprintf ( 1, '\n' );
    fprintf ( 1, 'GMSH_WRITE - Error!\n' );
    fprintf ( 1, '  Could not open the output file.\n' );
    error ( 'GMSH_WRITE - Error!' );
  end
%
%  Write the data.
%
